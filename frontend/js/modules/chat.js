...
function ChatModule() {
  ...
  const handleWebSocketClose = () => {
    ...
    if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
      ...
      appendSystemMessage(`Desconectado do servidor. Tentando reconectar em ${delay / 1000} segundos...`)
      ...
      appendSystemMessage("Não foi possível reconectar ao servidor. Por favor, recarregue a página.")
    }
  }
  ...
  const sendMessage = (event) => {
    ...
    if (!websocket || websocket.readyState !== WebSocket.OPEN) {
      appendSystemMessage("Não foi possível enviar a mensagem. Conexão perdida.")
      return
    }
    ...
    if (userId === currentUser.id) {
      ...
      messageElement.innerHTML = `
        ...
        <div class="message__bubble">
          <div class="message__sender" style="color: ${userColor}">${escapeHTML(userName)}</div>
          <div class="message__content">${escapeHTML(content)}</div>
          <div class="message__time">${timestamp}</div>
        </div>
      `
